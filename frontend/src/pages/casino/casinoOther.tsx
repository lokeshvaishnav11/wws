...
const CasinoOther = () => {
  ...
  React.useEffect(() => {
    casinoService
      .getIntCasinoList(category, queryp.type)
        ...
        <div className="row row5">
              ...
              {provider.map((Item: any, key: number) => {
                return (
                  <li className="nav-item" key={key}>
                    <CustomLink
                      className={`nav-link ${
                        Item._id == category || (!category && key == 0)
                          ? "active"
                          : ""
                      }`}
                      to={`/casino-int/${queryp.type}?category=${Item._id}`}
                    >
                      <span>{Item._id}</span>
                    </CustomLink>
                  </li>
          ...
          <div className="col-xl-10 col-12">
            ...
            <div className="casino-list mt-2" style={{ marginLeft: "-6px" }}>
              {games.length > 0 && (
                <CasinoListItemInt games={games} setGamePlay={setGamePlay} />
              )}
            </div>
          </div>
        </div>
      ...
      {gamePlay?.game_name && (
        <div className="slot-iframe show">
          <div className="slot-header">
            ...
            <div
              className="close-slot-frame"
              onClick={() => setGamePlay(undefined)}
            >
              EXIT
            </div>
          </div>
